	...
	LPSTR lpCmdLine, int nCmdShow)
{
	...
	wc.lpszClassName = TEXT("ANIMATE");

	g_hInst = hInstance;

	if (RegisterClass(&wc) == 0) return FALSE;

	mainwindow = CreateWindow(TEXT("animate"), TEXT("animate"), WS_CLIPCHILDREN | WS_CLIPSIBLINGS |
		WS_POPUP | WS_VISIBLE,
	...
	running = true;
	time1 = timeGetTime();
	tm.load("D:\\xt\\NV_example\\src\\examples\\MEDIA\\data\\textures.dat");
