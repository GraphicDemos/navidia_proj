...
ScotopicFilter::ScotopicFilter(): _nGamma(1.25f)
{
    ...
    if (_goScotopicShader == 0)
    {
        ...
        _goScotopicShader = cgCreateProgramFromFile(ShaderManager::gCgContext, 
                CG_SOURCE, 
                SHADER_PATH "cg_scotopic/Scotopic.cg",
                cgFragmentProfile(), 
                0, 
                0
             );
...
ScotopicFilter::image()
{
    ...
    return oOutputImage;
		...
		/*
        ImageFBO
ScotopicFilter::imageFBO()
{
    ImageFBO oOutputImage;

    if (_pSourceOperator != 0)
    {
        ImageFBO oBlurryImage   = _oBlurFilter.imageFBO();
        ImageFBO oBlurrierImage = _oBlurrierFilter.imageFBO();
