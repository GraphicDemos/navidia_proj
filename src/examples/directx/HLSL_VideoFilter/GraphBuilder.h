...
private: 
    ...
    CComPtr<IPin>           pFDecOutput;     // Decodder Filter Output Pin
...
public:
    ...
    HRESULT InitVMR9Renderer(HWND hWindow, IDirect3D9* d3d, IDirect3DDevice9* d3dd, Mutex *pMutex );
    HRESULT GetVMR9Texture(IDirect3DTexture9 **texture);

    HRESULT InitDShowTextureRenderer(HWND hWindow, LPDIRECT3DDEVICE9 pd3dDevice, WCHAR *wFileSource, Mutex *pMutex );
    ...
    HRESULT SetAllocatorPresenter( HWND window, IDirect3D9* d3d, LPDIRECT3DDEVICE9 d3dd, Mutex *pMutex );

    HRESULT CreateTextureSurface ( LPDIRECT3DDEVICE9 pd3dDevice, 
    ...
    BOOL IsMCActive() { return (g_pMC != NULL); }
};
