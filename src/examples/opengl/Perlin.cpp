...
typedef float noise_vector3[3];
typedef float noise_vector2[2];
...
{
	float p = (float)(log(1. - b) / log(0.5f));
	...
	if (a < .001)
		...
		return 1.;
	if (a < 0.5)
		return (float)(pow(2 * a, p) / 2.0);
	else
		return (float)(1. - pow(2.0f * (1.0f - a), p) / 2.0f);
}
...
float turbulence2(float *v, float freq)
{
	float t, vec[2];
