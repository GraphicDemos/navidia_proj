...
char * read_text_file(const char * filename, const char * type, unsigned long hModule)
{
	...
	else
	{
		...
		if (resource.res_type_id)
		{
			hr = FindResource(resource.hModule, wcstring, (LPCWSTR)resource.res_type_id);
		}
		else
		{
			size_t newsize = resource.res_type_name.length();
			wchar_t * wcstring = new wchar_t[newsize];
			size_t convertedChars = 0;
			mbstowcs_s(&convertedChars, wcstring, newsize, resource.res_type_name.c_str(), _TRUNCATE);
			hr = FindResource(resource.hModule, wcstring, wcstring);
		}
