...
void
writeToGPUBufferLocal(GPUBuffer *gp, float4 *src, int x, int y, int w, int h)
{
    setDstGPUBuffer(gp);
    glRasterPos2i(x, y);
...
GPUBuffer *
createGPUBufferLocal(int width, int height, int bitsPerComponent, int numComponents)
{
    GPUBuffer *gp;
    int     format;
...
void calcAmbientOcclusion(int passes)
{
    ...
    if (!obuf0) {   // initialize buffers
        obuf0 = createGPUBufferLocal(w, 256, 32, 4);
        obuf1 = createGPUBufferLocal(w, 256, 32, 4);
        createShader(OCCLUSION_SHADER, "..//..//..//..//media//programs//dynamic_amb_occ//occlusion.fp");
        createShader(OCCLUSION2_SHADER, "..//..//..//..//media//programs//dynamic_amb_occ//occlusion2.fp");
    }
