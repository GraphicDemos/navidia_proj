...
DLLExport BOOL WINAPI UIProc(HWND hDlg, UINT wMsg, WPARAM wParam, LPARAM lParam)     // Win32 Change
{
	...
	switch  (wMsg)
	{
      case WM_INITDIALOG:
		...
		SetCheckBoxState (hDlg, dConvertAlpha, gConvertAlpha && gHostIsPhotoshop401orLater);
		...
		if (gHostIsPhotoshop401orLater)
			...
			PIDisableControl (hDlg, dConvertAlpha);
				...
				if (cmd == BN_CLICKED)
				{
					...
					if (Validate(globals, hDlg, &lastItem))
					{
						...
						if (Validate(globals, hDlg, &lastItem))		
						{
							...
							gConvertAlpha = gHostIsPhotoshop401orLater &&
								GetCheckBoxState(hDlg, dConvertAlpha);
							state = STATE_AGAIN;
						} // bad cols
					} // bad rows
				} // btn clicked
