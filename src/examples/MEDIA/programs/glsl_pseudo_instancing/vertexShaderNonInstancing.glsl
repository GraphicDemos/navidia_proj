...
// Uniform variables
uniform vec4 worldMatrix[3];        // World Matrix
...
void main(void)
{
  ...
  normalWorld.x = dot(worldMatrix[0].xyz, gl_Normal);
  normalWorld.y = dot(worldMatrix[1].xyz, gl_Normal);
  normalWorld.z = dot(worldMatrix[2].xyz, gl_Normal);
