...
INT WINAPI WinMain( HINSTANCE, HINSTANCE, LPSTR, int )
{
    ...
    DXUTCreateWindow( L"StencilShadow" );
                                  ...
                                  D3DFORMAT BackBufferFormat, bool bWindowed, void* pUserContext )
{
	...
	if( !(pCaps->StencilCaps & D3DSTENCILCAPS_INCRSAT ))
		nErrors++;
...
bool CALLBACK ModifyDeviceSettings( DXUTDeviceSettings* pDeviceSettings, const D3DCAPS9* pCaps, void* pUserContext )
{
	...
	// See MSFT ShadowVolume demo for a more robust method of getting stencil in the depth buffer
...
HRESULT CALLBACK OnCreateDevice( IDirect3DDevice9* pd3dDevice, const D3DSURFACE_DESC * pBackBufferSurfaceDesc, void* pUserContext )
{
    ...
    //--------------------------------------
	SAFE_DELETE( g_pDemo );
	g_pDemo	 = new StencilShadowDemo;
	FAIL_IF_NULL( g_pDemo );
	hr = g_pDemo->CreateSysMemMeshes();
	//--------------------------------------

	D3DDeviceAndHWInfo hwi;
	hwi.Initialize( pd3dDevice );
	FMsg("Color buffer desc:\n");
	hwi.ReportSurfaceDesc( pBackBufferSurfaceDesc );
	D3DSURFACE_DESC DepthDesc;
	hwi.GetDepthBufferDesc( & DepthDesc, pd3dDevice );
	FMsg("Depth buffer desc:\n");
	hwi.ReportSurfaceDesc( & DepthDesc );
                                ...
                                const D3DSURFACE_DESC* pBackBufferSurfaceDesc, void* pUserContext )
{
    ...
    //---------------------------------
	MSG_AND_RET_VAL_IF( g_pDemo == NULL, "g_pDemo is NULL in OnResetDevice!\n", E_FAIL );
	hr = g_pDemo->Initialize( pd3dDevice );

	SAFE_DELETE( g_pTestStencil );
	g_pTestStencil = new TestStencil;
	FAIL_IF_NULL( g_pTestStencil );
	hr = g_pTestStencil->Initialize( pd3dDevice );
	//---------------------------------
	...
	g_Camera.SetResetCursorAfterMove( false );		// windowed
    g_Camera.SetScalers( 0.005f, 3.0f );
...
void CALLBACK OnFrameRender( IDirect3DDevice9* pd3dDevice, double fTime, float fElapsedTime, void* pUserContext )
{
    ...
    if (SUCCEEDED(pd3dDevice->BeginScene()))
    {
		...
		// g_pTestStencil displays a simple test pattern to see if stencil is working properly
		if( g_pTestStencil )
		{
//			g_pTestStencil->Render();
		}
...
void CALLBACK KeyboardProc( UINT nChar, bool bKeyDown, bool bAltDown, void* pUserContext )
{
	if( g_pDemo != NULL )
	{
		g_pDemo->KeyboardProc( nChar, bKeyDown, bAltDown );
	}
}
