...
//				*ptr++ = 0.333f*(r+g+b)/m_max;
		...
		if(!AUTOMIPMAP_FLAG){
			...
			for(unsigned int j = 1; j < m_pCubeMapF->GetLevelCount(); j++){
...
void HDRTexture::FillHalfCubemaps(){
	...
	for(int faceID = D3DCUBEMAP_FACE_POSITIVE_X; faceID <= D3DCUBEMAP_FACE_NEGATIVE_Z; faceID++){
		...
		if(!AUTOMIPMAP_FLAG){
			...
			for(unsigned int j = 1; j < m_pCubeMapA->GetLevelCount(); j++){
			...
			for(unsigned int j = 1; j < m_pCubeMapB->GetLevelCount(); j++){
