...
static void loadControlMesh(HWND hwnd)
{
    OPENFILENAME ofn;
    char *str;
    static TCHAR result[257];
   
    AnimatedVertices = 0;
    memset((void *) &ofn, 0, sizeof(ofn));
    ofn.lStructSize = sizeof(ofn);
    ofn.hwndOwner = hwnd;
    ofn.lpstrFilter = _T("Animation Files\0*.ANI\0Object Files\0*.OBJ\0All Files\0*.*\0\0");
    ...
    if (GetOpenFileName(&ofn)) {
		...
		wcstombs_s(&nNumCharConverted, szString, 128,
			ofn.lpstrFile, 128);

		str = getExtension(szString);
        if (!strcmp(str, "ANI") || (!strcmp(str, "ani")))
            readAnimationFile(szString);
        else {
            OpenMesh();
            read_model(szString);
            CloseMesh();
...
static void initialize()
{
	...
	if ((strstr((const char*)pExtString, "GL_NV_fragment_program2 ") == NULL) ||
		(strstr((const char*)pExtString, "GL_NV_fragment_program2 ") == (char*)pExtString))
	{
		printf("Required extension not found:  GL_NV_fragment_program2\nPress any key to exit\n");
		getch();
		exit(0);
	}
    ...
    init_shader("..//..//..//..//media//programs//dynamic_amb_occ//surface.fp", SURFACE_SHADER, GL_FRAGMENT_PROGRAM_NV);
}
...
WindowProc(HWND hwnd, UINT message, WPARAM wParam, LPARAM lParam)
{
    ...
    switch (message) {
        case WM_CREATE:
            ...
            readAnimationFile("..//MEDIA//models//dynamic_amb_occ//bigguy.ani");
    ...
    int nCmdShow)
{
    WNDCLASS    wc;
    MSG         msg;
    static TCHAR ClassName[] = _T("Window");
    static TCHAR Title[] = _T("DynamicAO");
