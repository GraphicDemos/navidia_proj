...
DECLSPEC_NV_MATH mat4 inverse(mat4 &mat);
DECLSPEC_NV_MATH void identity(mat4 &mat);

//TODO: ajeitar ou remover essa fun?ao
DECLSPEC_NV_MATH void mult_matrix_vec(const vec3 &src, vec3 &dst, mat4 &mat);
DECLSPEC_NV_MATH void mult_matrix_vec(vec3 & src_and_dst, mat4 &mat);

DECLSPEC_NV_MATH const vec4 operator*(const mat4&, const vec4&);
DECLSPEC_NV_MATH const vec4 operator*(const vec4&, const mat4&);
...
DECLSPEC_NV_MATH const quat operator*(const quat&, const quat&);
DECLSPEC_NV_MATH quat & add_quats(quat & p, const quat & q1, const quat & q2);
DECLSPEC_NV_MATH nv_scalar dot(const quat & p, const quat & q);
DECLSPEC_NV_MATH quat & dot(nv_scalar s, const quat & p, const quat & q);
DECLSPEC_NV_MATH quat & slerp_quats(quat & p, nv_scalar s, const quat & q1, const quat & q2);
DECLSPEC_NV_MATH quat & axis_to_quat(quat & q, const vec3 & a, const nv_scalar phi);
DECLSPEC_NV_MATH mat3 & quat_2_mat(mat3 &M, const quat &q );
DECLSPEC_NV_MATH quat & mat_2_quat(quat &q,const mat3 &M);
...
DECLSPEC_NV_MATH vec2 & normalize(vec2 & u);
DECLSPEC_NV_MATH vec3 & normalize(vec3 & u);
DECLSPEC_NV_MATH vec4 & normalize(vec4 & u);
...
// i.e.     u = v cross w
DECLSPEC_NV_MATH vec3 & cross(vec3 & u, const vec3 & v, const vec3 & w);

// computes the dot product ( v dot w) and stores the result in u
// i.e.     u = v dot w
DECLSPEC_NV_MATH nv_scalar & dot(nv_scalar & u, const vec3 & v, const vec3 & w);
DECLSPEC_NV_MATH nv_scalar dot(const vec3 & v, const vec3 & w);
DECLSPEC_NV_MATH nv_scalar & dot(nv_scalar & u, const vec4 & v, const vec4 & w);
DECLSPEC_NV_MATH nv_scalar dot(const vec4 & v, const vec4 & w);
DECLSPEC_NV_MATH nv_scalar & dot(nv_scalar & u, const vec3 & v, const vec4 & w);
DECLSPEC_NV_MATH nv_scalar dot(const vec3 & v, const vec4 & w);
DECLSPEC_NV_MATH nv_scalar & dot(nv_scalar & u, const vec4 & v, const vec3 & w);
DECLSPEC_NV_MATH nv_scalar dot(const vec4 & v, const vec3 & w);
...
//                     \|/
//                      +
DECLSPEC_NV_MATH vec3 & reflect(vec3 & r, const vec3 & n, const vec3 & l);

// Computes u = v * lambda + u
DECLSPEC_NV_MATH vec3 & madd(vec3 & u, const vec3 & v, const nv_scalar & lambda);
// Computes u = v * lambda
DECLSPEC_NV_MATH vec3 & mult(vec3 & u, const vec3 & v, const nv_scalar & lambda);
// Computes u = v * w
DECLSPEC_NV_MATH vec3 & mult(vec3 & u, const vec3 & v, const vec3 & w);
// Computes u = v + w
DECLSPEC_NV_MATH vec3 & add(vec3 & u, const vec3 & v, const vec3 & w);
// Computes u = v - w
DECLSPEC_NV_MATH vec3 & sub(vec3 & u, const vec3 & v, const vec3 & w);

// Computes u = u * s
DECLSPEC_NV_MATH vec2 & scale(vec2 & u, const nv_scalar s);
DECLSPEC_NV_MATH vec3 & scale(vec3 & u, const nv_scalar s);
DECLSPEC_NV_MATH vec4 & scale(vec4 & u, const nv_scalar s);

// Computes u = M * v
DECLSPEC_NV_MATH vec3 & mult(vec3 & u, const mat3 & M, const vec3 & v);
DECLSPEC_NV_MATH vec4 & mult(vec4 & u, const mat4 & M, const vec4 & v);

// Computes u = v * M
DECLSPEC_NV_MATH vec3 & mult(vec3 & u, const vec3 & v, const mat3 & M);
DECLSPEC_NV_MATH vec4 & mult(vec4 & u, const vec4 & v, const mat4 & M);

// Computes u = M(4x4) * v and divides by w
DECLSPEC_NV_MATH vec3 & mult_pos(vec3 & u, const mat4 & M, const vec3 & v);
// Computes u = M(4x4) * v
DECLSPEC_NV_MATH vec3 & mult_dir(vec3 & u, const mat4 & M, const vec3 & v);
// Computes u = M(4x4) * v and does not divide by w (assumed to be 1)
DECLSPEC_NV_MATH vec3 & mult(vec3& u, const mat4& M, const vec3& v);

// Computes u = v * M(4x4) and divides by w
DECLSPEC_NV_MATH vec3 & mult_pos(vec3 & u, const vec3 & v, const mat4 & M);
// Computes u = v * M(4x4)
DECLSPEC_NV_MATH vec3 & mult_dir(vec3 & u, const vec3 & v, const mat4 & M);
// Computes u = v * M(4x4) and does not divide by w (assumed to be 1)
DECLSPEC_NV_MATH vec3 & mult(vec3& u, const vec3& v, const mat4& M);

// Computes A += B
DECLSPEC_NV_MATH mat4 & add(mat4 & A, const mat4 & B);
DECLSPEC_NV_MATH mat3 & add(mat3 & A, const mat3 & B);

// Computes C = A + B
DECLSPEC_NV_MATH mat4 & add(mat4 & C, const mat4 & A, const mat4 & B);
DECLSPEC_NV_MATH mat3 & add(mat3 & C, const mat3 & A, const mat3 & B);

// Computes C = A * B
DECLSPEC_NV_MATH mat4 & mult(mat4 & C, const mat4 & A, const mat4 & B);
DECLSPEC_NV_MATH mat3 & mult(mat3 & C, const mat3 & A, const mat3 & B);

// Compute M = -M
DECLSPEC_NV_MATH mat4 & negate(mat4 & M);
DECLSPEC_NV_MATH mat3 & negate(mat3 & M);

// Computes B = Transpose(A)
//       T
//  B = A
DECLSPEC_NV_MATH mat3 & transpose(mat3 & B, const mat3 & A);
DECLSPEC_NV_MATH mat4 & transpose(mat4 & B, const mat4 & A);

// Computes B = Transpose(B)
//       T
//  B = B
DECLSPEC_NV_MATH mat3 & transpose(mat3 & B);
DECLSPEC_NV_MATH mat4 & transpose(mat4 & B);

// Computes B = inverse(A)
//       -1
//  B = A
DECLSPEC_NV_MATH mat4 & invert(mat4 & B, const mat4 & A);
DECLSPEC_NV_MATH mat3 & invert(mat3 & B, const mat3 & A);
...
//  B = A
DECLSPEC_NV_MATH mat4 & invert_rot_trans(mat4 & B, const mat4 & A);

DECLSPEC_NV_MATH mat4 & look_at(mat4 & M, const vec3 & eye, const vec3 & center, const vec3 & up);
DECLSPEC_NV_MATH mat4 & frustum(mat4 & M, const nv_scalar l, const nv_scalar r, const nv_scalar b, 
               const nv_scalar t, const nv_scalar n, const nv_scalar f);

DECLSPEC_NV_MATH mat4 & perspective(mat4 & M, const nv_scalar fovy, const nv_scalar aspect, const nv_scalar n, const nv_scalar f);
DECLSPEC_NV_MATH mat4 & ortho(mat4 & M, const nv_scalar left, 
...
*/
DECLSPEC_NV_MATH void decomp_affine(const mat4 & A, vec3 & T, quat & Q, quat & U, vec3 & S, nv_scalar & f);
// quaternion
DECLSPEC_NV_MATH quat & normalize(quat & p);
DECLSPEC_NV_MATH quat & conj(quat & p);
DECLSPEC_NV_MATH quat & conj(quat & p, const quat & q);
DECLSPEC_NV_MATH quat & add_quats(quat & p, const quat & q1, const quat & q2);
DECLSPEC_NV_MATH quat & axis_to_quat(quat & q, const vec3 & a, const nv_scalar phi);
DECLSPEC_NV_MATH mat3 & quat_2_mat(mat3 &M, const quat &q );
DECLSPEC_NV_MATH quat & mat_2_quat(quat &q,const mat3 &M);
DECLSPEC_NV_MATH quat & mat_2_quat(quat &q,const mat4 &M);

// surface properties
DECLSPEC_NV_MATH mat3 & tangent_basis(mat3 & basis,const vec3 & v0,const vec3 & v1,const vec3 & v2,const vec2 & t0,const vec2 & t1,const vec2 & t2, const vec3 & n);
...
DECLSPEC_NV_MATH nv_scalar nv_random();

DECLSPEC_NV_MATH quat & trackball(quat & q, vec2 & pt1, vec2 & pt2, nv_scalar trackballsize);

DECLSPEC_NV_MATH vec3 & cube_map_normal(int i, int x, int y, int cubesize, vec3 & v);
...
DECLSPEC_NV_MATH nv_scalar nv_area(const vec3 & v1, const vec3 & v2, const vec3 &v3);
// computes the perimeter of a triangle
DECLSPEC_NV_MATH nv_scalar nv_perimeter(const vec3 & v1, const vec3 & v2, const vec3 &v3);
// find the inscribed circle
DECLSPEC_NV_MATH nv_scalar nv_find_in_circle( vec3 & center, const vec3 & v1, const vec3 & v2, const vec3 &v3);
// find the circumscribed circle
DECLSPEC_NV_MATH nv_scalar nv_find_circ_circle( vec3 & center, const vec3 & v1, const vec3 & v2, const vec3 &v3);

// fast cosine functions
DECLSPEC_NV_MATH nv_scalar fast_cos(const nv_scalar x);
DECLSPEC_NV_MATH nv_scalar ffast_cos(const nv_scalar x);

// determinant
nv_scalar det(const mat3 & A);

DECLSPEC_NV_MATH void nv_is_valid(const vec3& v);
DECLSPEC_NV_MATH void nv_is_valid(nv_scalar lambda);
