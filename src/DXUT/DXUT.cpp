...
protected:
    struct STATE
    {
		...
		LPDXUTCALLBACKD3D9MODIFYDEVICESETTINGS m_IsD3D9ModifyDeviceSettingsFunc; // modify Direct3D d3d9 device settings callback

		LPDXUTCALLBACKISD3D9DEVICEFRAMEMOVE m_IsD3D9DeviceFrameMoveFunc;
		...
		LPDXUTCALLBACKISD3D10DEVICEFRAMEMOVE m_IsD3D10DeviceFrameMoveFunc;
		...
		void* m_IsD3D9ModifyDeviceSettingsFuncUserContext;     // user context for modify Direct3D d3D9 device settings callback

		void* m_IsD3D9DeviceFrameMoveFuncUserContext;
		...
		void* m_IsD3D10DeviceFrameMoveFuncUserContext;
...
public:
	...
	GET_SET_ACCESSOR(LPDXUTCALLBACKD3D9MODIFYDEVICESETTINGS, IsD3D9ModifyDeviceSettingsFunc);

	GET_SET_ACCESSOR(LPDXUTCALLBACKISD3D9DEVICEFRAMEMOVE, IsD3D9DeviceFrameMoveFunc);
	...
	GET_SET_ACCESSOR(LPDXUTCALLBACKISD3D10DEVICEFRAMEMOVE, IsD3D10DeviceFrameMoveFunc);
	...
	GET_SET_ACCESSOR(void*, IsD3D9ModifyDeviceSettingsFuncUserContext);

	GET_SET_ACCESSOR(void*, IsD3D9DeviceFrameMoveFuncUserContext);
	...
	GET_SET_ACCESSOR(void*, IsD3D10DeviceFrameMoveFuncUserContext);
...
void WINAPI DXUTSetCallbackD3D9DeviceChanging(LPDXUTCALLBACKD3D9MODIFYDEVICESETTINGS pCallback, void* pUserContext)				{ GetDXUTState().SetIsD3D9ModifyDeviceSettingsFunc(pCallback); GetDXUTState().SetIsD3D9ModifyDeviceSettingsFuncUserContext(pUserContext); }

void WINAPI DXUTSetCallbackD3D9DeviceFrameMove(LPDXUTCALLBACKISD3D9DEVICEFRAMEMOVE pCallback, void* pUserContext)			   { GetDXUTState().SetIsD3D9DeviceFrameMoveFunc(pCallback); GetDXUTState().SetIsD3D9DeviceFrameMoveFuncUserContext(pUserContext);}
...
void WINAPI DXUTSetCallbackD3D10DeviceFrameMove(LPDXUTCALLBACKISD3D10DEVICEFRAMEMOVE pCallback, void* pUserContext)				{ GetDXUTState().SetIsD3D10DeviceFrameMoveFunc(pCallback); GetDXUTState().SetIsD3D10DeviceFrameMoveFuncUserContext(pUserContext); }
                          ...
                          bool bForceRecreate, bool bClipWindowToSingleAdapter )
{
	...
	//自己添加
	LPDXUTCALLBACKD3D9MODIFYDEVICESETTINGS pCallbackD3D9ModifyDeviceSettings = GetDXUTState().GetIsD3D9ModifyDeviceSettingsFunc();
	if (pCallbackD3D9ModifyDeviceSettings)
	{
		pCallbackD3D9ModifyDeviceSettings(pNewDeviceSettings,GetDXUTState().GetCaps(), GetDXUTState().GetIsD3D9ModifyDeviceSettingsFuncUserContext());
	}
...
void DXUTRender3DEnvironment9()
{
	...
	bool _use_X_frame_Move_Func = true;
    ...
    if( pCallbackFrameMove != NULL )
    {
        ...
        if( NULL == pd3dDevice ) // Handle DXUTShutdown from inside callback
            return;
		_use_X_frame_Move_Func = false;
    }

#if 1 //后加的
	if (_use_X_frame_Move_Func)
	{
		LPDXUTCALLBACKISD3D9DEVICEFRAMEMOVE pCallbackD3D9FrameMove = GetDXUTState().GetIsD3D9DeviceFrameMoveFunc();
		if (pCallbackD3D9FrameMove != NULL)
		{
			pCallbackD3D9FrameMove(pd3dDevice, fTime, fElapsedTime, GetDXUTState().GetIsD3D9DeviceFrameMoveFuncUserContext());
			pd3dDevice = DXUTGetD3D9Device();
			if (NULL == pd3dDevice) // Handle DXUTShutdown from inside callback
				return;
		}
