...
namespace GetFilePath
{
	...
	//从media中获取
	static tstring GetMediaFilePath(const tstring&strFileName, bool bVerbose)
	{
		TCHAR pwd[MAX_PATH];
		GetCurrentDirectory(MAX_PATH, pwd);

		return FindMediaFile(strFileName, pwd, bVerbose);
	}

	static tstring GetMediaFilePath(const tstring&strFileName)
	{
		return GetMediaFilePath(strFileName, true);
	}
