                                ...
                                const D3DSURFACE_DESC* pBackBufferSurfaceDesc, void* pUserContext )
{
	...
	hr = g_Demo.Initialize( pd3dDevice );
	MSG_AND_RET_VAL_IF( FAILED(hr), TEXT("Couldn't initialize membranedemo!\n"), E_FAIL );
...
void RenderText()
{
	...
	if( g_Demo.m_ptstrCurrentTextureFilename != NULL )
	{
		txtHelper.SetInsertionPos( 5, (int)(pd3dsdBackBuffer->Height * 0.9f - 30));
		tstring tstr;
		tstr = tstrPrintf( TEXT("Color ramp texture: %s"), g_Demo.m_ptstrCurrentTextureFilename->c_str() );
		txtHelper.DrawTextLine( tstr.c_str() );
		txtHelper.DrawTextLine( TEXT("SPC - Cycle textures") );
	}
...
void CALLBACK KeyboardProc( UINT nChar, bool bKeyDown, bool bAltDown, void* pUserContext )
{
	if( bKeyDown )
	{
		...
		{
		case VK_SPACE:
			g_Demo.UseNextColorRamp();
			break;
		case VK_F4:
			g_Demo.ReloadTextures();
			break;
		case VK_F1:
